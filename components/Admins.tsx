    ...
    <div className="mx-auto max-w-2xl">
        ...
        <Field>
          ...
          <InputGroup>
            ...
            <InputGroupAddon align="inline-end">
              <InputGroupButton type="submit" variant="brand" size="xs" disabled={submitting} aria-busy={submitting}>
      ...
      {admins === undefined ? (
        ...
        <ul className="divide-y divide-border rounded-lg border border-border">
          {admins.map((admin) => (
            <li key={admin._id} className="flex min-h-12 items-center justify-between gap-3 px-4 py-2 transition-colors hover:bg-surface">
              <span className="flex min-w-0 items-center gap-3">
                <span className="truncate font-mono text-sm">{admin.email}</span>
                {admin.isSelf ? <Badge variant="outline" className="eyebrow shrink-0 border-brand/40 text-brand">You</Badge> : null}
              </span>
              {admin.isSelf ? (
                <AlertDialog>
                  <AlertDialogTrigger render={<Button variant="ghost" size="icon-sm" aria-label={`Remove ${admin.email}`} className="shrink-0 text-muted-foreground" />}>
                    <X />
                  </AlertDialogTrigger>
                  <AlertDialogContent>
                    <AlertDialogHeader>
                      ...
                      <AlertDialogDescription>
                        You will immediately lose admin access, and only another admin can add you back.
                      </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
                      <AlertDialogCancel>Cancel</AlertDialogCancel>
                      <AlertDialogAction variant="destructive" onClick={() => handleRemove(admin._id)}>Remove me</AlertDialogAction>
                    </AlertDialogFooter>
                  </AlertDialogContent>
                </AlertDialog>
              ) : (
                <Button variant="ghost" size="icon-sm" aria-label={`Remove ${admin.email}`} onClick={() => handleRemove(admin._id)} className="shrink-0 text-muted-foreground">
                  <X />
                </Button>
              )}
            </li>
