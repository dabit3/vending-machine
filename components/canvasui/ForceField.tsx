...
export function createForceField(
  ...
  syncCanvasSize();
  // Read the page luminance up front so the light/dark blend mode is right
  // even in fallback (non html-in-canvas) mode, where uploadContent never runs.
  pageLum = readPageLum();
  ...
  // Theme toggles swap the page background; re-read its luminance so the
  // field switches between additive (dark) and inked (light) blending.
