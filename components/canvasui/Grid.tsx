...
export interface GridOptions {
  ...
  idleRipples?: number;
  /** Tile face color as [r, g, b] in 0-1 range when no page capture backs the tiles. */
  baseColor?: [number, number, number];
  /** Peak tile opacity when no page capture backs the tiles (0 to 1). */
  fillOpacity?: number;
}
...
void main () {
  ...
  if (uHasContent > 0.5) {
    ...
    content = vec4(
      mix(uBaseColor, uTint, clamp(uTintStrength, 0.0, 1.0)),
      liftAmt * clamp(uFillOpacity, 0.0, 1.0));
  }
...
export function createGrid(
  ...
  function render(trailCount: number, delta: number) {
    ...
    gl!.uniform3f(
      mainPass.uniforms.uBaseColor,
      config.baseColor[0],
      config.baseColor[1],
      config.baseColor[2],
    );
    gl!.uniform1f(mainPass.uniforms.uFillOpacity, config.fillOpacity);
